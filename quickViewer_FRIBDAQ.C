...
void next(Int_t eventID = -1) {
  fEventID = eventID;
  fPlot -> DrawPadplane(fEventID);
  fCvs = fPlot -> DrawSideview(fCore -> GetEventID());
}

void save()
{
    TString name = Form("figures/figure_quickview_run%d_event%d.png",fRunNo,fEventID);
    cout << name << endl;
    fCvs -> SaveAs(name);
}
...
void quickViewer_FRIBDAQ(Int_t runNo = 1013)
{
  ...
  //  cout << endl;
  //  cout << cYELLOW << "== Usage: " << cNORMAL << "RUN=" << cRED << "####" << cNORMAL << " root quickViewer_FRIBDAQ.C" << endl;
  ...
  if (!fUseSeparatedData) {
    fCore = new STCore(dataFile, kTRUE);
  } else {
    fCore = new STCore(kTRUE);
    ...
    while (dataFileWithPath.ReadLine(listFile)) {
      Int_t fileNumber = TString(dataFileWithPath(dataFileWithPath.Length() - 6, 2)).Atoi();
      if (fileNumber > 0)
